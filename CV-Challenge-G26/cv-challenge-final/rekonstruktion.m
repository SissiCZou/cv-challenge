...
%% Finden der korrekten Euklidischen Bewegung ?�ber die Positive-Tiefen-Bedingung
...
% 4 m??gliche Kombinationen aus T_1,2 und R_1,2, d.h. wir muessen 4 mal das
...
for i=1:4
    ...
    for j=1:N
        ...
        % Das GLS f�r die Tiefen lambda_2 l?sst sich auch folgenderma?en
        % aufstellen
        % M2b((j-1)*3+1:(j-1)*3+3,j)    = skew( x1_array(:,j) ) * R_all(:,:,i)' *  x2_array(:,j);
        % M2b((j-1)*3+1:(j-1)*3+3,N+1)  = -skew( x1_array(:,j) ) * R_all(:,:,i)' * T_all(:,i);
    end
    
    % Least-Squares-L??sung via Singul?�rwertzerlegung
    ...
    % [~,~,V2b] = svd(M2b,0);
    % lambdas2b = V2b(:,N+1);
    % lambdas2b = lambdas2b ./ lambdas2b(end); 

    % Summiere ?�ber die Vorzeichen der Tiefen
...
% W?�hle das richtige Paar (R,T) und die korrekten Tiefen aus
T       = T_all(:,index);
R       = R_all(:,:,index);
...
% Vektor mit gesch?�tzten Tiefen aus Ansicht 1
lambda1 = lambda(:,1);
